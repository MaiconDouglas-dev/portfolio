...
export default function Projects() {
  ...
  const getGlowColor = (idx: number) => {
    switch (idx) {
      ...
      case 1:
        return 'rgba(10, 132, 255, 0.22)'; // Cyan / Blue
      case 2:
        return 'rgba(139, 92, 246, 0.22)'; // Electric Violet
      case 3:
      default:
        return 'rgba(48, 209, 88, 0.22)'; // Emerald / Mint
    }
  };

  return (
    <section id="projects" className="py-28 border-t border-white/[0.06] relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          {/* Quick-Jump Stepper Pills */}
          <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-2xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl">
              ...
              return (
                <MagneticButton key={proj.id} strength={0.2}>
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-mono transition-all duration-300 cursor-pointer ${
                      isActive
                        ? 'bg-appleRed-600 text-white font-bold shadow-lg shadow-appleRed-500/30'
                        : 'text-neutral-400 hover:text-white hover:bg-white/[0.06]'
                    }`}
                  >
                    <span className="text-[10px] opacity-75">{proj.number}</span>
                    <span className="truncate max-w-[110px] sm:max-w-none">
                      {idx === 0 ? 'Clyvo (Vet)' : `Slot ${proj.number}`}
                    </span>
                    {proj.isPlaceholder && (
        ...
        <div className="space-y-14 relative pb-16">
          {projects.map((proj, idx) => {
            const isFirst = idx === 0;
            const glow = getGlowColor(idx);
            const stackingTopOffset = 88 + idx * 16; // Incremental sticky top offset creates the deck layering
            ...
            return (
              <div
                ...
                style={{ top: `${stackingTopOffset}px` }}
              >
                <FuturisticCard
                  ...
                  contentClassName="p-6 sm:p-9 space-y-6 relative overflow-hidden"
                >
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-3 py-1 rounded-full text-[11px] font-mono font-bold border ${
                          isFirst
                            ? 'bg-appleRed-500/10 text-appleRed-400 border-appleRed-500/30'
                  ...
                  {proj.metrics && proj.metrics.length > 0 && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 relative z-10">
                      {proj.metrics.map((m, mIdx) => (
                        <div
                          key={mIdx}
                          className="p-3 rounded-2xl bg-white/[0.03] border border-white/[0.06] backdrop-blur-md"
                        >
                  ...
                  {proj.isPlaceholder && (
                    <div className="p-4 rounded-2xl bg-black/60 border border-dashed border-white/[0.12] space-y-2 relative z-10">
                      <div className="flex items-center justify-between text-[11px] font-mono text-neutral-400">
                        <span className="flex items-center gap-1.5 text-amber-300">
                          <Sparkles size={13} />
                          {lang === 'pt' ? 'Slot Reservado para Novo Projeto' : 'Reserved Slot for New Project'}
                        </span>
                        <span className="text-[10px] text-neutral-400">src/data/projects.ts</span>
                      </div>
                      ...
                      <>
                        <MagneticButton strength={0.25}>
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.08] hover:bg-white/[0.14] text-white font-semibold text-xs border border-white/[0.12] hover:scale-[1.02] cursor-pointer transition-all"
                          >
                            <Github size={14} />
